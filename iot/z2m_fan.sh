...
SOURCE_TOPIC="home/power/fan"
TARGET_TOPIC="zigbee2mqtt/fan/set"
...
echo -e "${YELLOW}Starting MQTT Fan Relay... ${NC}"
...
do
    ...
    if [ -n "$state" ]; then
        if [ "$state" = "ON" ]; then
            echo -e "${GREEN}[$(date '+%Y-%m-%d %H:%M:%S')] Received ON - Turning fan ON${NC}"
            mosquitto_pub -h "$MQTT_HOST" -t "$TARGET_TOPIC" -m '{"state":"ON"}'
        elif [ "$state" = "OFF" ]; then
            echo -e "${RED}[$(date '+%Y-%m-%d %H:%M:%S')] Received OFF - Turning fan OFF${NC}"
