...
for fitfile in "$input_directory"/*.fit; do
    ...
    # Convert the .fit file to .gpx using gpsbabel and save it in the output directory
    gpsbabel -i garmin_fit -f "$fitfile" -o gpx -F "$output_directory/${basefilename}.gpx"
done

echo "Conversion complete! GPX files saved in '$output_directory'."
